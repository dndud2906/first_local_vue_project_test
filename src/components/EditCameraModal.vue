                    ...
                    <div>
                        <a-form :layout="formLayout" :form="form" @submit="handleSubmit">                
                            <a-form-item label="NAME" :colon="false"  :wrapper-col="{span:14}"  :label-col="{span:10}">
                                <a-input 
                                    :disabled="true"
                                    v-decorator="['cName', { initialValue:editCameraInfo.cName, rules: [{ required: true }] }]"
                                />
                            </a-form-item>          
                            <a-form-item  label="LABEL" :colon="false"  :wrapper-col="{span:14}"  :label-col="{span:9, offset:1}">
                                <a-input
                                    v-decorator="['label', {initialValue:editCameraInfo.label, rules: [{ required: true }] }]"
                                />
                            </a-form-item>
                            <!-- -->
                            ...
                            <a-form-item label="OBJECT TYPE" :colon="false"  :wrapper-col="{span:14}"  :label-col="{span:10}">
                                <a-input
                                    v-decorator="['objType', { initialValue:editCameraInfo.objType, rules: [{ required: true }] }]"
                                />
                            </a-form-item>          
                            <a-form-item  label="RTSP URI" :colon="false"  :wrapper-col="{span:14}" :label-col="{span:9, offset:1}">
                                <a-input
                                    v-decorator="['rtspURI', { initialValue:editCameraInfo.rtspURI, rules: [{ required: true }] }]"
                                />
                            </a-form-item>
                            <!-- -->
                            ...
                            <a-collapse  :bordered="false">
                                <a-collapse-panel header="추가옵션" key="1" class="test">
                                    <!-- -->
                                    ...
                                    <a-form-item label="FPS" :colon="false" :style="{'width':'32%'}" :label-col="{span:14}" :wrapper-col="{span:10}">
                                        <a-input 
                                            v-decorator="['fps', { rules: [{ required: false }] }]"
                                        />
                                    </a-form-item>
                                ...
                                <!-- -->
                                    ...
                                    <a-form-item label="RETENT PERIOD" :colon="false" :style="{'width':'32.5%'}" :label-col="{span:17}" :wrapper-col="{span:7}">
                                        <a-input
                                            v-decorator="['period',{rules:[{required:false}] }]"
                                        />
                                    </a-form-item>
                            ...
                            <a-form-item :style="{marginLeft:'38%'}">
                                <a-button class="btnStyle" @click="editCancelClick">Cancel</a-button>
...
<script>
import {editCamera} from '../repository'
export default {
    name: 'EditCameraModal',
    props:['editModalVisible', 'editCameraInfo'],
    data(){
        return{
            formLayout:"inline",
            form: this.$form.createForm(this, {name:'CameraEditWindow'}),
            isActive:this.editModalVisible,
        }
    },
    methods:{
        editCancelClick(){
            ...
            this.isActive=false;
            this.$emit("setVisible","edit", this.isActive)
        },
        handleSubmit(e){
        e.preventDefault();
        this.form.validateFields((err, values) => {
            if (!err) {         
                console.log('Received editValues of form: ', values);
                editCamera(values, this.editCameraInfo._id)
                    .then(data => {
                        this.form.resetFields();
                        this.isActive=false;
                        this.$emit('editCamera', data.camera);
                        this.$emit('setVisible', "edit", this.isActive);
                    })
